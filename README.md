...
- [x] **技能市场**：从遵循 `.claude-plugin/marketplace.json` 标准的公开仓库实时拉取目录，卡片浏览 + 按市场筛选 + 搜索 + **点卡片看详情**（SKILL.md 全文轻量 Markdown 渲染 / frontmatter / 文件列表；插件展示其内含技能 + README）+ 一键安装到任意来源。已接入官方 `anthropics/skills`（17）+ 社区 `netresearch`（40）/`xiaolai`（15）。两种结构都支持：in-repo 技能（按子目录抽取）与社区「插件」（source 指向外部仓库 → 下载该仓库、抽取其全部 SKILL.md）
- [ ] 按 source 分组的多来源同时浏览
- [ ] 从本地 `.zip` 导入
- [ ] 市场聚合更多社区源（claude-plugins-official 等需解析跨仓库 source）

### v0.3 — 1 周
- [x] Skill 市场聚合 browser（pull from known marketplaces）— 见 v0.2 ✅
